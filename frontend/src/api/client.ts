...
export const api = {
  contests: {
    ...
    create: (data: { name: string; description: string; uploadEndDate: string; ratingEndDate: string; reward: string }) =>
      http.post<Contest>('/api/contests', data, { headers: adminHeaders() }).then(r => r.data),
    update: (id: number, data: { name: string; description: string; uploadEndDate: string; ratingEndDate: string; reward: string }) =>
